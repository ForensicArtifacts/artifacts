...
| 0.0.5 | J.B. Metz | September 2016 | Added REGISTRY_VALUE_DATA source type.
|===
...
- type: REGISTRY_VALUE
 attributes:
   key_value_data_tuples:
     - {key: 'HKEY_LOCAL_MACHINE\Software\Microsoft\Windows\CurrentVersion\Run', value: 'WINDOWS', data_type: 'REG_SZ', data: 'c:\documents and settings\administrator\local settings\temp\test5.exe'}
----
...
Where `key_value_pairs` can contain the following values:

[cols="1,5",options="header"]
|===
| Value | Description
| key | Windows Registry key path
| value | Windows Registry value name
| data_type | Optional value data type
| data | Optional value data
|===

[NOTE]
The artifact definition is descriptive therefore it is up to the implementation on how to use the contents of `data_type` and `data`.
