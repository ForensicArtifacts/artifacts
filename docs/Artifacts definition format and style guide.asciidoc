...
[abstract]
== Summary
This guide contains a description of the forensics artifacts definitions as-is. 
...
:numbered:
== Background
A first version of the artifact definitions originate from the 
https://github.com/google/grr[GRR project] where it is used to more easily 
collect data of interest, e.g. specific files or Windows Registry keys. The 
goal of the format is not to provide an all encompassing way to describe 
forensic artifacts, but at least a fair portion of it.

The format should kept simple and straight forward, so that a digital forensic
analysist is able to easily write artifact definitions in anger without having
to rely on complex standards or tooling.

The format is descriptive in nature and should be tied to a specific 
use case as little as possible. E.g. GRR uses the artifact definition
for collection but other projects could use them for annotation or just to
...
Where digital archaeological roughly refers to computer forensics without the 
...
Ideally the artifact definition links to an article that discusses the artificat in more depth e.g. forensicswiki.org
|===
...
*Style not*: explicit newlines (\n) should not be used.

== [[sources]]Sources
Every source definition start with a `type` followed by arguments e.g.
...
form should be used to save on line breaks.
