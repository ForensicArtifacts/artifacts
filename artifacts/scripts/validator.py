...
class ArtifactDefinitionsValidator:
    ...
    _SUPPORTED_POSIX_USERS_VARIABLES = ["%%users.homedir%%"]
        ...
        for path in paths:
            ...
            for path_segment in path_segments:
                if "**" in path_segment:
                    if has_globstar:
                        logging.warning(
                            f"Unsupported path: {path:s} with multiple globstars "
                            f"defined by artifact definition: "
                            f"{artifact_definition.name:s} in file: {filename:s}"
                        )
                        result = False
                        break
    ...
    def _CheckWindowsPath(self, filename, artifact_definition, source, path):
        ...
        if (
            number_of_forward_slashes < number_of_backslashes
        ...
        for path_segment in path_segments:
            if path_segment.startswith("%%") and path_segment.endswith("%%"):
                if (
                    path_segment.startswith("%%environ_")
                    and path_segment
                    not in self._SUPPORTED_WINDOWS_ENVIRONMENT_VARIABLES
                ):
                    result = False
                    logging.warning(
                        f"Artifact definition: {artifact_definition.name:s} in file: "
                    ...
                    path_segment.startswith("%%users.")
                    and path_segment not in self._SUPPORTED_WINDOWS_USERS_VARIABLES
                ):
                    result = False
                    logging.warning(
                        f"Artifact definition: {artifact_definition.name:s} in file: "
    ...
    def _CheckWindowsRegistryKeyPath(self, filename, artifact_definition, key_path):
        ...
        for segment_index, key_path_segment in enumerate(key_path_segments):
            if key_path_segment.startswith("%%") and key_path_segment.endswith("%%"):
                if (
                    segment_index == 1
                    and key_path_segment == "%%users.sid%%"
                    and key_path_segments[0] == "hkey_users"
                ):
                    continue
                ...
                if key_path_segment.startswith("%%environ_"):
                    ...
                    logging.warning(
                        f"Artifact definition: {artifact_definition.name:s} in file: "
                        f"{filename:s} contains Windows Registry key path that "
                        f"contains an environment variable: '{key_path_segment:s}'. "
                        f"Usage of environment variables in key paths is not "
                        f"encouraged at this time."
                    )
                    ...
                    logging.warning(
                        f"Artifact definition: {artifact_definition.name:s} in file: "
                        f"{filename:s} contains Windows Registry key path that "
                        f"contains a users variable: '{key_path_segment:s}'. Usage "
                        f"of users variables in key paths, except for "
        ...
        try:
            for artifact_definition in artifact_reader.ReadFile(filename):
                try:
                    ...
                    logging.warning(
                        f"Duplicate artifact definition: {artifact_definition.name:s} "
                        f"in file: {filename:s}"
                    )
                    result = False

                current_name_lower = artifact_definition.name.lower()
                heapq.heappush(
                    sorted_names, (current_name_lower, artifact_definition.name)
                )
                ...
                artifact_definition_supports_macos = (
                    definitions.SUPPORTED_OS_DARWIN
                    in (artifact_definition.supported_os)
                )
                artifact_definition_supports_windows = (
                    definitions.SUPPORTED_OS_WINDOWS
                    in (artifact_definition.supported_os)
                )
                ...
                for source in artifact_definition.sources:
                    ...
                    if source.type_indicator in (
                        ...
                        if definitions.SUPPORTED_OS_DARWIN in source.supported_os or (
                            artifact_definition_supports_macos
                            and not source.supported_os
                        ):
                            if source.separator != "/":
                                logging.warning(
                                    f"Use of unsupported path segment separator in "
                                    f"artifact definition: "
                                    f"{artifact_definition.name:s} in file: "
                                    f"{filename:s}"
                                )
                        ...
                        if (
                            ...
                            and self._HasDuplicateRegistryKeyPaths(
...
def Main():
    ...
    args_parser.add_argument(
        ...
        default=None,
        help=(
            "path of the file or directory that contains the artifact " "definitions."
        ),
    )
